...
cd "$repo_root" || exit 0
tsc_output=$(pnpm --silent --filter '@noam-website/web' exec tsc --noEmit --skipLibCheck --pretty false "$rel_path" 2>&1)
tsc_exit=$?
  ...
  if [ -n "$filtered" ]; then
    {
      ...
      echo "$filtered"
      echo "[post-edit-typecheck] Run \`pnpm -F @noam-website/web typecheck\` for full project view."
    } >&2
  fi
fi
