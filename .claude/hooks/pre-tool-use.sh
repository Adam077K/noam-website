#!/usr/bin/env bash
# PreToolUse safety gate — Noam-website Phase 6 (2026-05-16)
#
...
" 2>/dev/null || printf '%s' "$payload" | awk -F'"' '/"command"/{print $4; exit}')
    ...
    if printf '%s' "$command" | grep -qE 'pip\s+install|pip3\s+install'; then
      block "pip install is blocked. Python deps are not part of the Noam-website stack. Confirm with the CEO if this is intentional."
    fi
