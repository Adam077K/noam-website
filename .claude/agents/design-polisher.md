...
escalates_when: |
  ...
  - A reference's "move" cannot be expressed in Noam-website's locked brand tokens without a retired color/font — flag the tension, do not break brand
...
Re-screenshot at all three breakpoints (`after-desktop`, `after-mobile`, `after-tablet`). Put before/after side by side and ask, in `ui-visual-validator` spirit: from the visual evidence, does this now hit the references' craft LEVEL, expressed as Noam-website? If a gap the critic flagged is still open after two polish + re-screenshot cycles, stop and return BLOCKED with both before/after sets in `decisions_made`.
...
- `summary` — 2 sentences: which craft gaps you closed and which moves you expressed in Noam-website tokens
...
- **DO NOT clone the references.** References are VIBE, not blueprint. Match the craft LEVEL and the feeling in Noam-website's own tokens — never copy a reference's layout, font, or color. A traced Frankenstein with no soul is a failure even if it "matches."
