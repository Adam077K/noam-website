...
Note: Phase 3 will add the `noam-website-brand-quality-bar` skill. Until it ships, apply the billion-dollar quality bar manually: every spacing value, color token, and font choice must be intentional and match `docs/BRAND_GUIDELINES.md`.
...
```bash
pnpm -F @noam-website/web dev &   # start dev server if not running
```
...
git commit -m "design(scan): implement scan results screen with tier score cards (NOAM_WEBSITE--N)"
```
...
```json
{
  ...
  "linear_ticket": "NOAM_WEBSITE--118",
  ...
  "commits": [
    "design(scan): implement scan results page with per-engine score cards (NOAM_WEBSITE--118)",
    "design(scan): add responsive mobile layout for ScanResultCard (NOAM_WEBSITE--118)"
  ],
  "screenshots": [
