...
```bash
pnpm -F @noam-website/web test --listTests   # or grep for existing test files touching the changed code
```
...
```bash
pnpm -F @noam-website/web test apps/web/__tests__/<specific-file>.test.ts
```
...
git commit -m "test(api/agents): add coverage for credit-guard execution path (NOAM_WEBSITE--N)"
```
...
```json
{
  "status": "COMPLETE",
  "agent": "qa-engineer",
  "linear_ticket": "NOAM_WEBSITE--117",
  ...
  "commits": [
    "test(api/agents): add 6 tests for credit-guard execution path — 401, 402, 422, happy (NOAM_WEBSITE--117)"
  ],
