export const meta = {
  name: 'qa',
  description: 'Noam-website T5 binding QA gate — parallel dimension reviewers, 3 adversarial verifiers on block-eligible findings only (P1 always; P2 at irreversible — P3/advisory reported unverified), Opus judge emits PASS/BLOCK with a deterministic P1-override. A BLOCK stops the merge; the CEO cannot override (only Adam, via a logged false-positive appeal). A failed correctness/security review is an automatic coverage-gap BLOCK. Irreversible tier adds loop-until-dry finder rounds.',
  phases: [
...
const DIMENSIONS = [
  ...
  { key: 'patterns', critical: false, lens: 'Noam-website conventions (repo conventions, Zod on inputs, TS strict, error handling, no placeholder UI), naming, dead code, duplication' },
...
function reviewPrompt(d, attempt) {
  return `You are reviewing a Noam-website diff for the **${d.key}** dimension only.
...
function verifyPrompt(f, lensIndex) {
  ...
  return `Adversarially verify ONE claimed QA finding against the real Noam-website diff (\`git diff ${REF}\`).
...
function judgePrompt(confirmed, tier, failedDims, advisory) {
  return `You are the binding QA-Lead judge for a Noam-website **${tier}** change. Diff range: ${REF}.
