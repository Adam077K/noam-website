export const meta = {
  name: 'coding',
  description: 'Noam-website T5 complex-coding workflow — implements independent slices in parallel isolated worktrees, then chains the combined diff into the binding qa.js gate. Returns per-slice results + the QA verdict. Does NOT merge — merge is Adam-gated after a qa.js PASS.',
  phases: [
...
function buildPrompt(s) {
  return `You are implementing ONE focused Noam-website coding slice in an isolated git worktree branched from origin/main.
Slice [${s.id}] — ${s.brief}
