...
git commit -m "test(api/scan): add unit tests for /api/scan/start auth + validation (NOAM_WEBSITE--104)"
```
...
```json
{
  ...
  "commits": [
    "test(api/scan): add 8 unit tests for /api/scan/start auth, validation, and happy path (NOAM_WEBSITE--104)"
  ],
  ...
  "decisions_made": [
    {
      ...
      "reason": "Playwright sandbox not configured in CI for this branch; QA-Lead approved unit-only scope for NOAM_WEBSITE--104"
    }
  ],
  "blockers": []
}
