...
git commit -m "feat(db): add rate_limits table with RLS for per-IP scan throttling (NOAM_WEBSITE--104)"
```
...
```json
{
  ...
  "commits": [
    "feat(db): add rate_limits table with RLS for per-IP scan throttling (NOAM_WEBSITE--104)"
  ],
