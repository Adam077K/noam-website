...
git commit -m "feat(ai/scan): implement scan-analyzer agent with eval (NOAM_WEBSITE--112)"
```
...
```json
{
  ...
  "commits": [
    "feat(ai/scan): implement scan-analyzer agent — extracts mention sentiment from AI search results (NOAM_WEBSITE--112)",
...
|---|---|
| RAG retrieval / pgvector queries | `pgvector-rag-noam-website` |
