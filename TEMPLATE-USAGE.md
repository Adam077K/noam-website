...
|-------------|---------|---------|
| `Noam-website` | Display name in agent prompts and brand voice | `Acme` |
| `noam-website` | lower-case slug used in env vars / urls | `acme` |
| `NOAM_WEBSITE` | UPPER prefix for env vars (e.g. `ACME_API_KEY`) | `ACME` |
| `NOAM_WEBSITE-` | Linear / Jira ticket prefix | `ACME-` |
| `noam-website.com` | Production domain | `acme.com` |
| `https://noam-website.com` | Marketing site URL (often a separate platform) | `https://acme.com` |
| `https://github.com/Adam077K/noam-website` | GitHub repo URL | `https://github.com/org/repo` |
...
| `.claude/hooks/*.sh` and `*.js` | Hooks run on every action. Read each one before enabling. `gsa-context-monitor.js` references env var `NOAM_WEBSITE_NO_AUTOCOMPACT` — set it or remove the check. |
