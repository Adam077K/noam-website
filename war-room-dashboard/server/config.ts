...
export const CONFIG = {
  port: 4200,
  clientDevPort: 4201,
  projectStateDir: join(HOME, '.noam-website'),
  projectDir: join(HOME, 'VibeCoding', 'Noam-website'),
  worktreesDir: join(HOME, 'VibeCoding', 'Noam-website', '.worktrees'),
  claudeProjectsDir: join(HOME, '.claude', 'projects'),
  tmuxSession: 'noam-website',
  intervals: {
    state: 2000,
    tmux: 3000,
    events: 1000,
